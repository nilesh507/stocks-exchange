...
export class Engine {
        ...
        switch (message.type) {
            ...
            case CANCEL_ORDER:
                try {
                    ...
                    // const baseAsset = cancelMarket.split("_")[1];
                    ...
                    if (order.side === "buy") {
                        ...
                        const leftQuantity = (order.quantity - order.filled) * order.price; // if order have been filled partially, need to calculate the left quantity and increase the user balance by the amount(left quantity)
                        //@ts-ignore
    ...
    // pushes the filled order by a userID to db_processor Redis-queue
    ...
    checkAndLockFunds(baseAsset: string, quoteAsset: string, side: "buy" | "sell", userId: string, asset: string, price: string, quantity: string) {
        if (side === "buy") {
            // user should have money to buy the quoteassets[INR, $, etc] 
    ...
    onRamp(userId: string, amount: number) {
        ...
        if (!userBalance) {
            this.balances.set(userId, {
                [BASE_CURRENCY]: {
                    available: (this.balances.get(userId)?.[BASE_CURRENCY].available || 0 ) + amount, // want to increment the amount of the user
                    locked: 0
                }
            });
        } else {
